...
pred_all <- append(pred$Low, pred$High)
...
for (i in 1:10000) {
  ...
  expenses <- lease + seismic + complete
  ...
  co_ip_decline <- U %*% t(ip_decline)
  co_ip_decline <- t(co_ip_decline)

  final_ip_decline <- cbind(destandardize(co_ip_decline[,2], ip))
  
  for(y in 1:years){
    revenue = rep(1, years)
    
    # decline <- final_ip_decline[i,2]
    # ip <- final_ip_decline[i,1]
  ...
  sum_revenue = rep(0, years)
  for (r in revenue){
    m <- revenue[r]/1+1.1*(P6)
  }
