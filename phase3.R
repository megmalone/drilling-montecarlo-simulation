...
npv_dist <- rep(1, num_simulations)
years <- 5

for (i in 1:num_simulations) {
  # The total number of planned wells is assumed to be a uniform distribution between 10 and 30
  tot_wells <- floor(runif(1, min = 10, max = 30)) # Added floor() so value must be a whole number
  ...
  well_results <- rbinom(tot_wells, 1, prob)
  
  prop_wet <- sum(well_results) / length(well_results)
  prop_dist[i] <- prop_wet
  
  cnt0 <- sum(well_results == 0) # Number of predicted dry wells
  cnt1 <- sum(well_results == 1) # Number of predicted wet wells
  
  dry_well_costs <- rep(1, cnt0)
  for (w in 1:cnt0) {
    ...
    prof <- rtriangle(n = 1,
                      ...
                      b = 279000,
                      c = 215000) / 1000
    other_expenses <- lease + seismic + prof
    ...
    dry_well_costs[w] <- (P6 + other_expenses) * 1000
  }
  ...
  for (a in 1:cnt1) {
    ...
    final.ip_decline.r <-
      cbind(destandardize(ip_decline.r[, 1], decline),
    ...
    for (y in 1:years) {
      ...
      revenue <- income - opcost - prof
      wacc <- (1.1) ** y
      
      if (a == 1){
        annual_npv[y] <- revenue - expenses / wacc
      }
      else{
        annual_npv[y] <- revenue / wacc 
      }
      
    }
    wet_npv[a] <- sum(annual_npv)
  }
  
  npv <- -sum(dry_well_costs) + sum(wet_npv)
  npv_dist[i] <- (npv / 1000000)
}

hist(npv_dist)

# Provide histogram of proportion of wells producing, probability of hydrocarbons, probability of resevoir.
hist(prop_dist)
...
# 5% CVaR from this distribution
CVaR <- mean(VaR_data)

# 5% VaR of NPV after 15 years
VaRnpv <- unname(quantile(npv_dist, c(0.05)))[[1]]
# Filter data to only bottom 5%
VaR_data_npv <- npv_dist[npv_dist <= VaRnpv]
# 5% CVaR from this distribution
CVaR_npv <- mean(VaR_data_npv)
